...
MAX_ADVERSE_DRIFT_TICKS = 2   # reject staircase downtrends that fake oscillation
ENABLE_DRIFT_FILTER = False   # see note below

# Entry must follow a FRESH downward move, not merely a low price. Measured:
# entering on a fresh cross won 81-90% across soccer/basketball/tennis against
# 35-49% for entries at a random moment in the same band. Being cheap is not
# the signal; having just moved cheap is.
# DISABLED. The standalone measurement (81-90% on a fresh level cross vs 35-49%
# for random entry) did not survive implementation: as a live gate it produced
# 0.451-0.526 win rate, at or below the 0.454 unfiltered baseline. The gap is
# that the standalone test entered once per cross, while a gate fires on every
# update inside the window, at progressively worse prices. Set ENABLE_CROSS_GATE
# True to re-test it.
ENABLE_CROSS_GATE = False

CROSS_WINDOW_SECS = 30        # how recently the cross must have happened

# The measured signal is a cross DOWN THROUGH a specific level, not merely
# "price is below its recent high" - the latter is true all the way down a
# sustained decline, which is exactly what loses money. These are the levels
# the excursion tracker marks.
CROSS_LEVELS = [Decimal(x) for x in ("0.05", "0.08", "0.10", "0.15", "0.20", "0.25", "0.30")]

# Entry must still be NEAR the crossed level. Measured win rate by how far
# below the level the entry sits: 1 tick 63-71%, 2 ticks 69-77%, 5 ticks
# 76-88%. The deep-gap numbers look best but require having been filled
# mid-gap, which is exactly the fill we cannot rely on, so cap it tight.
CROSS_MAX_TICKS_BELOW = 2
...
class PriceHistory:
    ...
    def crossed_down(self, key, now, window=CROSS_WINDOW_SECS):
        """Did the price cross DOWN through one of CROSS_LEVELS in `window`?

        Returns the level crossed, or None. A sustained decline only counts at
        the moment it passes a level, so this does not fire repeatedly all the
        way down the way a "below recent high" test does.
        """
        dq = self.pts.get(key) or deque()
        recent = [(ts, px) for ts, px in dq if now - ts <= window]
        if len(recent) < 2:
            return None
        now_px = recent[-1][1]
        for i in range(1, len(recent)):
            ...
            for lvl in CROSS_LEVELS:
                if prev >= lvl > cur:
                    # only while price is still close to the level it crossed
                    if (lvl - now_px) / TICK <= CROSS_MAX_TICKS_BELOW:
                        return lvl
        return None

    def net_drift_ticks(self, key) -> Decimal:
        """Signed move from the start of the window to now, in ticks.

        Negative means the price has trended against a long entry. Oscillation
        counts alone cannot tell a staircase downtrend from a genuine
        two-sided market.
        """
        seq = self._changes(key)
        if len(seq) < 2:
            return Decimal("0")
        return (seq[-1] - seq[0]) / TICK
...
def evaluate(entry: Decimal, target: Decimal, our_qty: Decimal, exit_levels,
    ...
    m["crossed"] = hist.crossed_down(key, now) if ENABLE_CROSS_GATE else True
    if ENABLE_CROSS_GATE and m["crossed"] is None:
        reasons.append(f"no level cross within {CROSS_MAX_TICKS_BELOW} ticks "
                       f"in {CROSS_WINDOW_SECS}s")

    m["drift"] = hist.net_drift_ticks(key)
    if ENABLE_DRIFT_FILTER and m["drift"] <= -MAX_ADVERSE_DRIFT_TICKS:
        reasons.append(f"downtrend: net {m['drift']:.0f} ticks over window")
